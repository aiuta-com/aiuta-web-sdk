...
const recentlyPhotosFromStorage: Array<ImagesTypes> =
  typeof window !== 'undefined' && JSON.parse(localStorage.getItem('tryon-recent-photos') || '[]')
...
export const generateSlice = createSlice({
  ...
  reducers: {
      ...
      if (Array.isArray(payload)) {
        ...
        if (state.selectedImages.length > 0) {
          if (state.selectedImages.includes(payload)) {
            const removeExistingImageId = state.selectedImages.filter((id) => id !== payload)
    ...
    setRecentlyPhotos: (state, { payload }) => {
      ...
      localStorage.setItem('tryon-recent-photos', JSON.stringify(recentlyPhotosFromStorage))
    },
    setGeneratedImage: (state, { payload }) => {
      ...
      localStorage.setItem('generated-images-history', JSON.stringify(generatedImagesFromStorage))
    },
  },
})

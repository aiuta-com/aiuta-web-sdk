...
export class ShowFullScreenModal {
  ...
  private changeActiveImage(image: ImageType): void {
    const imageModalContent = document.getElementById('aiuta-image-modal-content')
    const generatedImages = document.getElementById('aiuta-generated-images-view')

    const hasGeneratedImages = generatedImages && typeof generatedImages === 'object'

    const hasImageModalContent = imageModalContent && typeof imageModalContent === 'object'
  ...
  private calculateLeftContentScroll(): void {
    const generatedImagesView = document.getElementById('aiuta-generated-images-view')
    if (!generatedImagesView) return

    if (this.images && this.images.length > 0) {
      const activeImageIndex = this.images.findIndex((image) => image.id === this.activeImage.id)
  ...
  private miniImageView(image: ImageType, onClick: (image: ImageType) => void): any {
  ...
  private handleDeleteImages(): void {
    if (this.activeImage && this.images) {
      ...
      if (deleteActiveImage.length) {
        const deleteModal = document.getElementById('aiuta-delete-active-modal')
        const leftContent = document.getElementById('aiuta-generated-images-view')
  ...
  private makeDeleteImageModal(): void {
    ...
    if (hasAiutaModal) {
      ...
      Object.assign(paragraph.style, DELETE_IMAGE_MODAL_TEXT as CSSStyleDeclaration)

      Object.assign(modalDiv.style, DELETE_IMAGE_MODAL_BOX as CSSStyleDeclaration)

      Object.assign(buttonLine.style, DELETE_MODAL_BUTTON_CONTENT as CSSStyleDeclaration)
      ...
      deleteButton.addEventListener('click', this.handleDeleteImages.bind(this))
    }
  }
  ...
  private makeLeftContent(): void {
    ...
    Object.assign(leftContentDiv.style, LEFT_CONTENT_STYLES as CSSStyleDeclaration)
  ...
  private makeMiddleContent(): void {
    ...
    if (hasAiutaModal) {
      ...
      if (this.modalType) {
        ...
        trashDiv.addEventListener('click', this.makeDeleteImageModal.bind(this))
      }
  ...
  closeModal(): void {
    ...
    const hasAiutaModal = aiutaModal && typeof aiutaModal === 'object'
    const hasModalAndBodyhasAiutaModal = hasAiutaModal && document && document.body
